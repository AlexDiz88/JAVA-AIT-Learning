...
public class Another_card_game {
    public static boolean game(int[] frankCards, int[] samCards, int[] tomCards) {
        if (frankCards[2] > samCards[0] && frankCards[2] > tomCards[0]) {
            if (frankCards[3] > samCards[1] && frankCards[3] > tomCards[1]) {
                return true;
            }
        }
        return false;
